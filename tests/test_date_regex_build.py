...
class TestDateRegexBuild(unittest.TestCase):
    ...
    def test_build_regex_with_slash(self):
        ...
        self.assertEqual(
            result, '[1-31]{1,2}[/][1-12]{1,2}[/]\\d{2} [00-23]{2}[:][00-59]{2}[:][00-59]{2}')

    def test_build_regex_with_date_format_only(self):
        date = Date()
        result = date._build_regex('D-M-Y')
        self.assertTrue(result, '[00-31]{1,2}[-][00-12]{1,2}[-]\\d{4}')
    ...
    def test_build_regex_with_date_of_double_digits(self):
        ...
        result = date._build_regex('D/M/Y h:i:s')
        self.assertEqual(
            result, '[01-31]{1,2}[/][01-12]{1,2}[/]\\d{4} [00-23]{2}[:][00-59]{2}[:][00-59]{2}')
